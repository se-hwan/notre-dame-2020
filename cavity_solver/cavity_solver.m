...
%--- Variables for file handling (All files are globally accessible) ---
...
global u;               % Solution vector [p, u, v]^T at each node (current iteration)
global uold;            % Solution vector (previous iteration)
...
iterout = 5000;         % Number of time steps between solution output (output every "iterout" time steps)
...
cfl  = 0.8;             % CFL number used to determine time step
...
rkappa = 0.5;           % Time derivative preconditioning ant
...
pinf = 0.801333844662;  % Initial pressure (N/m^2) -> from MMS value at cavity center (No need to change)
...
dt = zeros(imax,jmax);              % Local time step at each node
artviscx = zeros(imax,jmax);        % Artificial viscosity in x-direction
artviscy = zeros(imax,jmax);        % Artificial viscosity in y-direction
u = zeros(imax,jmax,neq);           % Solution vector (p, u, v)^T at each node
uold = zeros(imax,jmax,neq);        % Previous (old) solution vector
s = zeros(imax,jmax,neq);           % Source term
...
if (irstr==0)           % Starting run from scratch
    ...
    if (irstr==1)  % Restarting from previous run (file 'restart.in')
        ...
        ninit = fscanf(fp4, '%d', [1, 1]); % Need to known current iteration 
        rtime = fscanf(fp4, '%f', [1, 1]); % Need to known current time value
        resinit = fscanf(fp4, '%f %f %f', [3, 1]); % Needs initial iterative residuals for scaling
        for j=1:jmax
            for i=1:imax
                readCol = fscanf(fp4, '%lf %lf %lf %lf %lf', [5, 1]);
                x = readCol(1); y = readCol(2);
                u(i,j,1) = readCol(3); u(i,j,2) = readCol(4); u(i,j,3) = readCol(5);
            end
        end               
        ninit = ninit + 1;
...
for j=2:jmax-1
    for i=2:imax-1
        ...
        u(i,j,1) = uold(i,j,1)-beta2*dt(i,j)*(rho*dudx+rho*dvdy-(artviscx(i,j)+artviscy(i,j))-s(j,i,1));
        u(i,j,2) = uold(i,j,2)-dt(i,j)*rhoinv*(rho*uold(i,j,2)*dudx+rho*uold(i,j,3)*dudy+dpdx-rmu*d2udx2-rmu*d2udy2-s(j,i,2));            
        u(i,j,3) = uold(i,j,3)-dt(i,j)*rhoinv*(rho*uold(i,j,2)*dvdx+rho*uold(i,j,3)*dvdy+dpdy-rmu*d2vdx2-rmu*d2vdy2-s(j,i,3));
...
for j=jmax-1:-1:2
    for i=imax-1:-1:2
        ...
        u(i,j,1) = u(i,j,1)-beta2*dt(i,j)*(rho*dudx+rho*dvdy-(artviscx(i,j)+artviscy(i,j))-s(j,i,1));
        u(i,j,2) = u(i,j,2)-dt(i,j)*rhoinv*(rho*u(i,j,2)*dudx+rho*u(i,j,3)*dudy+dpdx-rmu*d2udx2-rmu*d2udy2-s(j,i,2));            
        u(i,j,3) = u(i,j,3)-dt(i,j)*rhoinv*(rho*u(i,j,2)*dvdx+rho*u(i,j,3)*dvdy+dpdy-rmu*d2vdx2-rmu*d2vdy2-s(j,i,3));
